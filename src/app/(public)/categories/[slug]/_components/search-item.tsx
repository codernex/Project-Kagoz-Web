...
import { appendApi, cn, removeHtmlTags, trimToWordCount } from "@/lib/utils";
    ...
    <div className="p-[2.4rem] drop-shadow-lg rounded-smd space-y-[2rem] xl:space-y-[3rem] bg-white flex flex-col md:flex-row items-end h-fit relative w-full">
      <div className="w-full md:w-[60%]">
          ...
          <div>
            <div className="relative w-fit">
              <Link
                ...
                className="text-smd lg:text-md font-bold leading-md text-black"
              >
                {business.name}
              </Link>
              {business.isTrusted ? (
            ...
            <div className="flex flex-col gap-y-2 md:flex-row md:gap-x-[3rem] mt-2">
              {business.startingDate ? (
                <p className="text-xs text-muted items-center flex space-x-[0.8rem]">
                  ...
                  <span>
                    {differenceInYears(
                      new Date(),
                      business.startingDate || subYears(new Date(), 1)
                    )}{" "}
                    years in business
                  </span>
                </p>
              ...
              <p className="text-xs text-muted flex space-x-[0.8rem]">
                ...
                <span>
                  {Math.round(
                    differenceInDays(new Date(), business.createdAt) / 30
                  )}{" "}
                  Month with KAGOZ
                </span>
              </p>
        ...
        <p className="text-xsm lg:text-sm text-muted">
          {trimToWordCount(removeHtmlTags(business.about), 10)}
        </p>
      </div>
